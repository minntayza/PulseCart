...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-3xl border border-border bg-surface shadow-[0_2px_10px_rgb(0,0,0,0.02)] transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgb(0,0,0,0.06)] hover:border-primary/30">
      <Link href={`/products/${product.id}`} aria-label={`View details for ${product.name}`} className={`relative block h-56 overflow-hidden bg-gradient-to-br ${visual.gradient}`} style={product.imageUrl ? { backgroundImage: `url(${product.imageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' } : { display: 'grid', placeItems: 'center' }}>
        ...
        {!product.imageUrl && <span className="relative z-10 select-none text-8xl font-black text-foreground/10 transition-transform duration-500 group-hover:scale-110">{visual.symbol}</span>}
