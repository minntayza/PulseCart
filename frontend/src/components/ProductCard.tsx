...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-border/80 bg-surface shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-card-hover">
      ...
      <div className="p-5">
        <h3 className="line-clamp-1 text-base font-bold tracking-tight text-foreground"><Link href={`/products/${product.id}`} className="hover:text-primary transition-colors">{product.name}</Link></h3>
        <p className="mt-1.5 line-clamp-1 text-sm leading-5 text-text-secondary">{product.description}</p>
